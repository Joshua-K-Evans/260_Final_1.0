...
MessageElement processReceivedData(const char *data) {
    ...
    if (strncmp(data, "+OK", 3) == 0) {
        ...
        strncpy(theElement.theBuffer,data+6,MAX_MESSAGE_LENGTH);
        ...
        if (sscanf(data, "+RCV=%d,%d,%[^,],%d,%d", &senderAddress, &messageLength, message, &rssi, &snr) == 5) {
            ...
            if (!strcmp("TOGGLE LED", message)){
            	osSemaphoreRelease(toggleLEDSemHandle);
            	osSemaphoreRelease(toggleLED2semHandle);
            }
